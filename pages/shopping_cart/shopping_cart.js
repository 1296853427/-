Page({
  ...
  switchSelect: function (e) {
    ...
    if (this.data.carts[index].isSelect) {
      this.data.totalMoney = (this.data.totalMoney + (this.data.carts[index].price * this.data.carts[index].count)).toFixed(2);
    } else {
      this.data.totalMoney = (this.data.totalMoney - (this.data.carts[index].price * this.data.carts[index].count)).toFixed(2);
    }
    //是否全选判断
    for (i = 0; i < this.data.carts.length; i++) {
      Allprice = (Allprice + (this.data.carts[index].price * this.data.carts[index].count)).toFixed(2);
    }
  ...
  allSelect: function (e) {
    ...
    if (!this.data.isAllSelect) {
      ...
      for (i = 0; i < this.data.carts.length; i++) {
        ...
        this.data.totalMoney = (this.data.totalMoney + (this.data.carts[i].price * this.data.carts[i].count)).toFixed(2);
    ...
    if (this.data.isAllSelect) {
      ...
      for (var i = 0; i < this.data.carts.length; i++) {
        if (this.data.carts[i].isSelect){
          this.data.selectItem[cnt] = this.data.carts[i]
          cnt++
        }      
      }
